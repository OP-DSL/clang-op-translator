#ifndef OMP4MASTERKERNELHANDLER_HPP
#define OMP4MASTERKERNELHANDLER_HPP
...
class OMP4MasterKernelHandler : public matchers::MatchFinder::MatchCallback {
...
public:
  ...
  void addMatchersTo(clang::ast_matchers::MatchFinder &finder) {
    ...
    const StatementMatcher constCopyToGPUMatcher =
        ifStmt(hasCondition(unaryOperator(
                   hasUnaryOperand(ignoringImpCasts(callExpr())))),
  ...
  std::string generateConstCopies() {
    ...
    llvm::raw_string_ostream os(repl);
    for (const op_global_const &c : constants) {
      os << "if(!strcmp(name,\"" << c.name << "\")) { memcpy(" << c.name
         << "_ompkernel"
         << ", dat, dim*size);\n";
      os << "#pragma omp target enter data map(to:" << c.name << "_ompkernel";
      if (c.size != 1)
        os << "[:" << c.size << "]";
      os << ")\n} else ";
    }
  ...
  virtual void run(const matchers::MatchFinder::MatchResult &Result) override {
    if (!HANDLER(clang::IfStmt, 1, "if_copy",
                 OMP4MasterKernelHandler::generateConstCopies))
      return;
    if (!HANDLER(clang::VarDecl, 20, "kernel_includes",
                 OMP4MasterKernelHandler::generateIncludes))
      return;
    if (!HANDLER(clang::VarDecl, 11, "global_var",
                 OMP4MasterKernelHandler::generateConstDefs))
      return;
  }
};

} // namespace OP2
#endif /* ifndef OMP4MASTERKERNELHANDLER_HPP */
